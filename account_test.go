	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := s.AccountImport(tt.args.identifier, tt.args.nick, tt.args.faceUrl); (err != nil) != tt.wantErr {
				...
				t.Errorf("AccountImportApi() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
...
func TestIMServer_AccountCheck(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("AccountCheckApi() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("AccountCheckApi() got = %v, want %v", got, tt.want)
			}
		})
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := s.Kick(tt.args.userId); (err != nil) != tt.wantErr {
				t.Errorf("KickApi() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
...
func TestIMServer_QueryState(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("QueryStateApi() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("QueryStateApi() got = %v, want %v", got, tt.want)
			}
		})
	}
}
