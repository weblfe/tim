	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("GetNoSpeakingApi() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if gotC2CmsgNospeakingTime != tt.wantC2CmsgNospeakingTime {
				t.Errorf("GetNoSpeakingApi() gotC2CmsgNospeakingTime = %v, want %v", gotC2CmsgNospeakingTime, tt.wantC2CmsgNospeakingTime)
			}
			if gotGroupmsgNospeakingTime != tt.wantGroupmsgNospeakingTime {
				t.Errorf("GetNoSpeakingApi() gotGroupmsgNospeakingTime = %v, want %v", gotGroupmsgNospeakingTime, tt.wantGroupmsgNospeakingTime)
			}
		})
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := s.SetNoSpeaking(tt.args.account, tt.args.C2CmsgNospeakingTime, tt.args.groupmsgNospeakingTime); (err != nil) != tt.wantErr {
				t.Errorf("SetNoSpeakingApi() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
